...
> **前提**：本書のアルゴリズムは「**enabledness が解決済みの明示 LTS**」を対象とする。
> 記号モデル（自然言語ガード付き）では、各辺の実行可能性 `Enabled_e = Guard ∧ ∃v'.Post`
> （Post 充足不能な辺は disabled）と τ閉包・受理集合を**意味論バックエンドが先に解決**してから
> 本アルゴリズムを適用する。`Guard` だけで enabledness を近似してはならない（偽 PASS の原因）。
> 自然言語の扱い全般は本書のスコープ外（`PLAN.md` 参照）。
