...
@enduml
`)
	result := tracechk.Check(tracechk.MatchExact, d, tracechk.Trace{Name: "trace.tsv", Events: []csdf.Event{"x", "w", "y"}})
...
@enduml
`)
	result := tracechk.Check(tracechk.MatchExact, d, tracechk.Trace{Name: "trace.tsv", Events: []csdf.Event{"x", "x"}})
